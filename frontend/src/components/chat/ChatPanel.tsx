...
export default function ChatPanel({ isOpen, onClose, currentUserId }: ChatPanelProps) {
  ...
  const handleSendMessage = (content: string) => {
    ...
    // TODO: socketService.emit('message:send', { content, roomId: selectedConversationId });
  };
      ...
      {selectedConversationId && selectedConversation ? (
        <>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderBottom: '1px solid #222222' }}>
            ...
            <div style={{ flex: 1 }}>
              <p style={{ color: '#EEEEEE', fontSize: 13, fontWeight: 600 }}>{selectedConversation.friendName}</p>
              <p style={{ color: selectedConversation.isOnline ? '#50C878' : '#555', fontSize: 11 }}>{selectedConversation.isOnline ? 'Online' : 'Offline'}</p>
            </div>
          </div>

          <MessageList messages={currentMessages} currentUserId={currentUserId} isTyping={false} />
          <MessageInput onSend={handleSendMessage} />

          <div style={{ padding: '8px 16px', borderTop: '1px solid #222222' }}>
            <p style={{ color: '#444', fontSize: 11, textAlign: 'center' }}>TODO: conectar a Socket.io quando backend estiver pronto</p>
          </div>
        </>
      ) : (
        <ConversationList
