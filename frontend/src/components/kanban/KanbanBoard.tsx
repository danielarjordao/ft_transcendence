  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: '#111111' }}>
      ...
      <div style={{ padding: '0 20px', borderBottom: '1px solid #2A2A2A', display: 'flex', alignItems: 'center', gap: '4px', flexShrink: 0 }}>
        <button style={{ padding: '10px 12px', background: 'transparent', border: 'none', borderBottom: '2px solid #7B68EE', color: '#F5F5F5', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}>
          All
        </button>
        {subjects.map(s => (
          <button key={s.id} style={{ padding: '10px 12px', background: 'transparent', border: 'none', borderBottom: '2px solid transparent', color: '#888888', fontSize: '13px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px' }}>
            <span style={{ width: 7, height: 7, borderRadius: '50%', background: s.color }} />
            {s.name}
          </button>
        ))}
